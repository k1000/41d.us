 ...
 * This file inlines the script for bundling by wrangler at deploy time.
 *
 * NOTE: The ECDH P-256 + AES-256-GCM crypto logic here is an inline copy of
 * packages/sdk/src/crypto.ts and packages/sdk/src/sdk-crypto-session.ts.
 * The duplication exists because this script must be pipeable via `curl | node -`
 * with zero npm dependencies. Keep algorithm choices, base64url encoding, and
 * EncryptedBody format in sync. See apps/web/test/crypto-primitives.test.ts.
 */
export const clientScript: string = String.raw`#!/usr/bin/env node
